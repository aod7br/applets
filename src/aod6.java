...
public class aod6 extends java.applet.Applet {


URL url;
Image cinza,imagem;
//Thread runner;
int cx,cy,zero_x,zero_y,r,r2,largura,altura,n_linhas,n_vistas,n_passos,i_passos;
int[] pixels,pixels2; // o array da imagem e do resultado
float[][] pontos; // o resultado das integrais de cada ponto
float [][] p; // as integrais em cada linha/imagem
Font fonte=new Font("TimesRoman",Font.BOLD,14);
Integer it;
public Button start_button;
TextField entrada_linhas,entrada_vistas,entrada_passos;
Choice menu;
...
// o bloco abaixo e so para corrigir o dithering do netscape
  pg = new PixelGrabber(imagem,0,0,largura,altura,pixels,0,largura);
   try { pg.grabPixels(); }
    ...
    pontos=new float[largura+1][altura+1];
...
public void monta_painel() {
    ...
    this.add( new Label("numero de passos"));
    entrada_passos=new TextField(3);
    entrada_passos.setText(it.toString(n_passos));
    this.add(entrada_passos);
   ...
   switch(evento.id) {
       ...
       case evento.ACTION_EVENT: {
          if (evento.target==start_button) {
               ...
               n_passos=it.parseInt(entrada_passos.getText());
               if (n_passos>20) n_passos=20;
...
public void anima_retas (Graphics g, float angulo) {
   ...
   int x0,x1,x2,x3,y0,y1,y2,y3,i,j;
   float coss,sinn,dx;
