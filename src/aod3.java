...
public class aod3 extends java.applet.Applet {
...
public float qy(float y) {
    ...
    for (i=0;i<=MAX;i++) {
        integral+=x*(.54+.46*Math.cos(2*Math.PI*x/A))*Math.cos(2*Math.PI*x*y);
        x+=d;
    }
    
    integral=integral*d;

    return (float) (2*integral);
}
   ...
   x2=-r; y2=r; x3=r; y3=r;
      ...
      for (i=0;i<largura;i++) {
         for (j=0;j<altura;j++) {
            ...
            teta=(float)Math.atan2(x,y);
            x=rxy(i,j)*(float)Math.cos(angulo-teta-Math.PI/2);
...
public void desenha_imagem_integrada() {
   ...
   for(i=0;i<largura;i++) for (j=0;j<altura;j++) {
      pontos[i][j]=(float)(pontos[i][j]-min);
      if (pontos[i][j]>max) { max=pontos[i][j]; }
   }

   for(i=0;i<largura;i++) for (j=0;j<altura;j++) {
      x=pontos[i][j]/max;
      c=(int)Math.round(255*x);
